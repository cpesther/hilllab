...
if (nargin < 3 || isempty(board))  
    board = 'daqtest';             
    print_message('No board specified.  Using virtual board "daqtest" as default');   
end

if (nargin < 4 || isempty(channels))   
    channels = 0:(N-1);  
    print_message(['DACoperator: Using default AO channels: 0 to ', num2str(N-1)]);
end

if (nargin < 5 || isempty(srate))   
    srate  = 10000;	
    print_message(['Sampling rate defaulted to ', num2str(srate), 'Hz']);
end

if (nargin < 6 || isempty(Vrange))   
    Vrange = [-5 5];	
    print_message(['DACoperator: Voltage range defaulted as ', num2str(Vrange(1)), ' to ', num2str(Vrange(2)), ' volts']);
end
...
print_message(['Each waveform is ' num2str(waveform_period) ' seconds.']);
print_message(['The number of repeats is: ' num2str(Nrepeat)]);
print_message(['Full sequence period is: ' num2str(full_period) ' seconds.']);
...
if strcmp(board, 'PCIe-6738')
    ...
    try
        ...
        if contains(daqname, 'PCIe-6738')
            ...
            print_message('Channels configured as ...'); 
            dq.Channels
            ...
            print_message('Zeroing and reseting DAq.');
            start(dq,'continuous');
            ...
            daqreset;
            print_message('The DAq board was reset.');

        else
            % Detected board is not PCIe-6738, use virtual board instead
            print_message('The DAq board is not PCIe-6738. Using daqtest.');
            board = 'daqtest';
        end
    ...
    % Handle errors during board detection or signal sending
        ...
        if contains(daqname, 'PCIe-6738')
            ...
            print_message('Error sending signal with PCIe-6738. Defaulting to "daqtest".');
        else
            % No NI DAQ boards found, use virtual board
            print_message('No NI DAQ boards found. Defaulting to "daqtest" virtual board.');
        end
...
elseif strcmp(board, 'daqtest') 
    ...
    print_message('"daqtest" selected. Running virtual test.');
else
    % Board name does not match installed hardware, fallback to virtual board
    print_message('Board name does not match installed hardware. Defaulting to "daqtest" virtual board.');
    board = 'daqtest';
end
...
start_time = posixtime(datetime('now'));  % Start time, seconds since epoch

% Print out a bunch of information
print_message(['Board ID: ' board]);
print_message(['Sampling Rate: ' num2str(srate)]);
print_message(['Voltage Range: [' num2str(Vrange(1)) ' ' num2str(Vrange(2)) ']']);
print_message(['Number of repeats: ' num2str(Nrepeat)]);
print_message(['Writing to output channels: ' num2str(channels(:)')]);
print_message('Signals to send to DAQ channels are plotted.');        
...
try
    ...
    print_message('Input is too long. Shorten the experiment duration or the sampling rate.');
    v = 0;
    beep;     % Audible alert
...
if length(t_p) == length(fullinput)
    print_message('t_p is equal to fullinput'); 
end

% Print decimation factor used for plotting
print_message(['Plotting vector is decimated 1:' num2str(factor)]);
...
function outs = downsample(input, factor)
    outs = input(1:factor:end,:);
end